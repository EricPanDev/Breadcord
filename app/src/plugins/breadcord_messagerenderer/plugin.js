...
function resolveMessageAuthorColor(message, options = {}) {
  ...
  function renderAttachments(attachments = []) {
    ...
    attachments.forEach(att => {
      if (!att) return; const item = el('div', 'breadcord-message__attachment');
      const url = att.url || att.proxy_url || '';
      const isImage = /(png|jpe?g|gif|webp)$/i.test(url) || (att.content_type || '').startsWith('image');
      if (isImage && url) { const img = el('img'); img.src = url; img.loading = 'lazy'; item.appendChild(img); }
      const link = el('a', null, att.filename || url || 'Attachment'); link.href = url || '#'; link.target = '_blank'; link.rel = 'noopener noreferrer'; item.appendChild(link);
