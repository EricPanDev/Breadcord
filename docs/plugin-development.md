...
### Example: Using the Breadcord Message Renderer Utility

The `breadcord_messagerenderer` plugin exposes a small DOM utility for rendering Discord-style messages without immediately mounting them in the document. It ships with helper styles and can be consumed from any plugin once loaded.

```javascript
// Ensure the plugin is loaded (list it in BreadAPI.plugins)
const { renderMessage, createExampleMessage } = window.BreadcordMessageRenderer;

// Build message JSON (use your own data or the helper)
const demoMessage = createExampleMessage({
    content: 'Custom content for my plugin!'
});

// Get a DOM element representing the message
const messageNode = renderMessage(demoMessage);

// Append it wherever you like
document.querySelector('#some-container').appendChild(messageNode);
```

#### Expected message JSON shape

The renderer accepts plain objects with the following optional fields:

| Field | Type | Description |
| --- | --- | --- |
| `id` | `string` | Unique message identifier (used for `data-message-id`). |
| `author` | `object` | `{ username, displayName, avatarUrl }` (any missing values fall back gracefully). |
| `content` | `string` | Message body. Newlines are preserved. |
| `timestamp` | `string \| Date` | ISO timestamp displayed in the header. |
| `editedTimestamp` | `string \| Date` | Adds the `(edited)` tag when present. |
| `attachments` | `array` | Each attachment supports `{ url, filename, content_type }`. Images are previewed automatically. |
| `embeds` | `array` | Basic embed support (`title`, `url`, `description`, `fields`, `thumbnail`, `color`). |
| `reactions` | `array` | Objects like `{ emoji: { name, id }, count, me }` render reaction chips. |

The renderer never attaches nodes to the DOM—callers own the resulting element and can insert it where appropriate.
